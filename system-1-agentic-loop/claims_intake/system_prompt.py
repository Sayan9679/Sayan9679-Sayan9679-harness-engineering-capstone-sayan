...
# MANDATORY TERMINAL CONTRACT (CRITICAL)

Every claim intake session MUST conclude with an explicit terminal tool call:
- Under NO circumstances should you stop, conclude, or output text-only responses before invoking a terminal tool.
- DO NOT stop after merely looking up the policy or recording claim facts. You MUST continue in the same session to classify the claim, assess its severity, and invoke either `route_to_adjuster` or `escalate_to_human`.
- Only AFTER your terminal tool call has been executed and returned may you emit your final confirmation response to the claimant and end the turn.
...
- Ending the session before calling `route_to_adjuster` or `escalate_to_human` is strictly forbidden.
"""
