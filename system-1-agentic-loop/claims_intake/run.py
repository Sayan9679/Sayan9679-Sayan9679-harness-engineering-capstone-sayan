...
def main(argv: list[str] | None = None) -> int:
    ...
    for fx_path in fixtures_to_run:
        ...
        if row["outcome"] == "incomplete":
            any_failure = True
            print(
                f" !! {row['claim_id']} ended incomplete; rerun or strengthen the terminal prompt",
                file=sys.stderr,
            )
        ...
        if not result.session.terminal_called:
            any_failure = True
        # Rigorous post-run assertion: every claim session must invoke a terminal tool
        assert result.session.terminal_called, (
            f"Terminal action missing: claim {fixture['claim_id']} finished without calling "
            "route_to_adjuster or escalate_to_human"
        )
        print(
