...
### Example 3 — clean review (restraint / reviewer trust)
...
+  if (!order) {
+    throw new ApiError(404, "NOT_FOUND", `order ${id} not found`);
+  }
+  return order.status;
+}
```
...
```
verdict: approve
notes: clean change — correctly uses ordersRepo, awaits asynchronous calls, and adheres to team error handling standards with ApiError. No findings to report.
```
