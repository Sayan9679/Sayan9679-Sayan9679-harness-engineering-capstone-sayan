...
The warm database retains full historical defect records (with `WarmStore.count() == 42` defects in the complete store), while the Shift C run filtered this down via `WarmStore.defects_since()` to an indexed SQL slice of exactly 3 new defects (`shift C: 3 new defects`), demonstrating that the model never receives the full historical defect log.
