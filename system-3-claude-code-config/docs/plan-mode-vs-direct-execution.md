...
Use plan mode when a change crosses multiple files, has unclear dependencies, or requires investigation before implementation. For example, a change involving the files `docs/plan-mode-vs-direct-execution.md`, `src/services/payment.ts`, and `tests/test_us05_plan_mode_doc.py` should be planned first. The goal is to understand the affected surfaces, dependencies, and validation steps and to prevent costly rework.
